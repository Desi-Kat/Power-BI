# 📊 Power BI Analysis & Visualisations Repository

This repository contains **Power BI Desktop (.pbix) projects** showcasing a range of business intelligence and data analysis work. Each project focuses on real-world data scenarios, transforming raw data into **interactive dashboards, analytical insights, and decision-ready visualisations**.

---

## 🚀 Projects Included

### 1. **Sales Performance Analysis**
A comprehensive performance dashboard exploring sales metrics and team performance.

[LINK](https://app.powerbi.com/view?r=eyJrIjoiYWI5Nzg2OTYtODE3OS00NjhjLThhZGQtZWQwZDkyZWMyYzk1IiwidCI6IjNlYTdjMTI4LWM2MDEtNDQ3OS1hMDAzLWUxNGQwMGMwYjVjYiJ9)

**Dataset:**

There were 5 data sets used and data was transformed to ensure relevant columns are used for the aim of the analysis.

<img width="600" height="347" alt="image" src="https://github.com/user-attachments/assets/2e4521e0-2ad6-4b15-871c-4bbfb376a800" />

<img width="792" height="440" alt="image" src="https://github.com/user-attachments/assets/e69aaba5-6e92-476e-9f41-a97ec19f9ccf" />


**Key Focus Areas:**
- Total Sales & Total Profit
- Profit Ratios and Discount Patterns  
- Comparison of sales performance across individuals and regions  

**Highlights:**
- Use of calculated measures for performance metrics  
- Visual emphasis on high/medium/low performance indicators

---

### 2. **Total Sales Excluding Tax**

Visual analysis of annual sales and geographical distribution over four consecutive years.

<img width="836" height="471" alt="image" src="https://github.com/user-attachments/assets/0d8b7726-4e6d-4fdd-b43c-84e5e021e14a" />

<img width="571" height="536" alt="image" src="https://github.com/user-attachments/assets/667b571d-f4da-463d-8b4c-d10d76112a53" />

<img width="763" height="433" alt="image" src="https://github.com/user-attachments/assets/16a5929b-7584-457b-ac1b-1b204ef3ddbb" />

**Key Focus Areas:**
- Year-on-year sales trends
- Sales distribution by country and region  
- Drill-down hierarchy and interactive expand modes

**Highlights:**
- Data transformation and cleaning using Power Query  
- Map visuals and interactive charts for geographic insights

---


  **2.1 Sales Dashboard**

  [LINK](https://app.powerbi.com/view?r=eyJrIjoiZGNlZTFiMzYtMTBmNC00ODNiLTkyOTItNzYyZTRhNGY3ZmM1IiwidCI6IjNlYTdjMTI4LWM2MDEtNDQ3OS1hMDAzLWUxNGQwMGMwYjVjYiJ9)
  
  Using two more datasets as practice I have identified profit and sales by geographical location, segment and average discount. 

<img width="1483" height="810" alt="image" src="https://github.com/user-attachments/assets/66199a3e-055a-489e-9d00-feeec4578157" />


---

### 3. **Employee Training and Development Report and Performance Analysis**

[LINK](https://app.powerbi.com/view?r=eyJrIjoiZjY1ZTkwZWUtOTBjYi00ZjM3LTlmYzktNzRhZWU5NjMzNDViIiwidCI6IjNlYTdjMTI4LWM2MDEtNDQ3OS1hMDAzLWUxNGQwMGMwYjVjYiJ9)

<img width="893" height="501" alt="image" src="https://github.com/user-attachments/assets/4ba1a59d-5e1f-40a7-86d4-c15f48523ae7" />

<img width="895" height="506" alt="image" src="https://github.com/user-attachments/assets/b6db66c5-8ee0-4821-bd48-fffeef31cd5a" />


A dashboard designed around employee training and performance data to present participation, cost, and satisfaction metrics.

**Key Focus Areas:**
- Training cost by department
- Business unit performance and hierarchies  
- Satisfaction and engagement trends  

---

### 4. **Work-Based Projects – Learner Analysis**

Analysis created to support training delivery for data analytics programmes, showing trends and outcomes across cohorts.

**Key Focus Areas:**
- Cohort performance over time  
- Learner completion rates and analytics  
- Regional and programme-level comparisons

<img width="816" height="460" alt="image" src="https://github.com/user-attachments/assets/ca5b68f3-26bc-4f4b-a332-b1972e7f94c5" />

<img width="819" height="463" alt="image" src="https://github.com/user-attachments/assets/40e92a6b-ded0-404b-afc9-b36622014a57" />

<img width="818" height="462" alt="image" src="https://github.com/user-attachments/assets/d4e45f96-876a-4b41-b0fb-3508ba11b7dd" />

<img width="762" height="363" alt="image" src="https://github.com/user-attachments/assets/b86c0856-01d4-4faf-88ec-3cf5ee15a119" />

---

### 5. Combining Data from Excel and an OData feed in Power BI

An overview of products sold, geographic distribution and order date.

[LINK](https://app.powerbi.com/view?r=eyJrIjoiMDU0OTNjYTUtZTM2ZC00NjdiLTg4YjgtODhhNjMxMDU0NjJjIiwidCI6IjNlYTdjMTI4LWM2MDEtNDQ3OS1hMDAzLWUxNGQwMGMwYjVjYiJ9)

<img width="898" height="468" alt="image" src="https://github.com/user-attachments/assets/7c0f5d1b-983d-4371-8535-ae1a16fa1727" />

## 🛠 Skills & Tools Demonstrated

| Category | Skills |
|----------|--------|
| **BI Tools** | Power BI Desktop, Power Query |
| **Data Modelling** | Measures, Hierarchies, DAX calculations |
| **Visualization** | Custom dashboards, interactive charts |
| **Data Prep** | ETL in Power Query |
| **Storytelling** | Insight-driven dashboards for decision making |

---

